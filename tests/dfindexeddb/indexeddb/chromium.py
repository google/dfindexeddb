...
class ChromiumIndexedDBTest(unittest.TestCase):
  ...
  def test_data_version_key(self):
    ...
    expected_key = chromium.DataVersionKey(
        offset=4, key_prefix=chromium.KeyPrefix(
            offset=0, database_id=0, object_store_id=0, index_id=0))
    expected_value = 20

    record = ((b'\x00\x00\x00\x00\x02'), (b'\x14\x00\x00\x00\x0f'))
    parsed_key = chromium.DataVersionKey.FromBytes(record[0])
    parsed_value = parsed_key.ParseValue(record[1])
  ...
  def test_earliest_sweep_key(self):
    ...
    expected_key = chromium.EarliestSweepKey(
        offset=4, key_prefix=chromium.KeyPrefix(
            offset=0, database_id=0, object_store_id=0, index_id=0))
    expected_value = 13318143299762026

    record = ((b'\x00\x00\x00\x00\x05'), (b'j7s\xe0\xc7P/'))
    parsed_key = chromium.EarliestSweepKey.FromBytes(record[0])
    parsed_value = parsed_key.ParseValue(record[1])
  ...
  def test_earliest_compaction_key(self):
    ...
    expected_key = chromium.EarlistCompactionTimeKey(
        offset=4, key_prefix=chromium.KeyPrefix(
            offset=0, database_id=0, object_store_id=0, index_id=0))
    expected_value = 13318229420051176

    record = ((b'\x00\x00\x00\x00\x06'), (b'\xe8\x8a\x9e\xed\xdbP/'))
    parsed_key = chromium.EarlistCompactionTimeKey.FromBytes(record[0])
    parsed_value = parsed_key.ParseValue(record[1])
  ...
  def test_scopes_prefix_key(self):
    ...
    expected_key = chromium.ScopesPrefixKey(
        offset=4, key_prefix=chromium.KeyPrefix(
            offset=0, database_id=0, object_store_id=0, index_id=0))
    expected_value = b'\x08\x01'

    record = ((b'\x00\x00\x00\x002\x00'), (b'\x08\x01'))
    parsed_key = chromium.ScopesPrefixKey.FromBytes(record[0])
    parsed_value = parsed_key.ParseValue(record[1])
  ...
  def test_database_name_key(self):
    ...
    expected_key = chromium.DatabaseNameKey(
        offset=4, key_prefix=chromium.KeyPrefix(
            offset=0, database_id=0, object_store_id=0, index_id=0),
    ...
    record = ((
        ...
        b'\x00s\x00t'), (b'\x04'))
    parsed_key = chromium.DatabaseNameKey.FromBytes(record[0])
    parsed_value = parsed_key.ParseValue(record[1])
  ...
  def test_database_metadata_key(self):
    ...
    with self.subTest('ORIGIN_NAME'):
      expected_key = chromium.DatabaseMetaDataKey(
          offset=4,
          key_prefix=chromium.KeyPrefix(
      ...
      record = (b'\x00\x04\x00\x00\x00', b'\x00a')

      parsed_key = chromium.DatabaseMetaDataKey.FromBytes(record[0])
      parsed_value = parsed_key.ParseValue(record[1])
      self.assertEqual(expected_key, parsed_key)
      self.assertEqual(parsed_value, expected_value)
    ...
    with self.subTest('DATABASE_NAME'):
      ...
      record = (b'\x00\x04\x00\x00\x01', b'\x00t\x00e\x00s\x00t')
    ...
    with self.subTest('MAX_ALLOCATED_OBJECT_STORE_ID'):
      ...
      record = (b'\x00\x04\x00\x00\x03', b'\x02')
    ...
    with self.subTest('IDB_INTEGER_VERSION'):
      ...
      record = (b'\x00\x04\x00\x00\x04', b'\x01')
    ...
    with self.subTest('BLOB_NUMBER_GENERATOR_CURRENT_NUMBER'):
      ...
      record = (b'\x00\x04\x00\x00\x05', b'\x01')
  ...
  def test_index_metadata_key(self):
    ...
    with self.subTest('INDEX_NAME'):
      expected_key = chromium.IndexMetaDataKey(
          offset=4, key_prefix=chromium.KeyPrefix(
              offset=0, database_id=4, object_store_id=0, index_id=0),
      ...
      parsed_key = chromium.IndexMetaDataKey.FromBytes(record[0])
      parsed_value = parsed_key.ParseValue(record[1])
      self.assertEqual(expected_key, parsed_key)
      self.assertEqual(parsed_value, expected_value)
    ...
    with self.subTest('UNIQUE_FLAG'):
      ...
      record = (b'\x00\x04\x00\x00d\x01\x1f\x01', b'\x00')
      parsed_key = chromium.IndexMetaDataKey.FromBytes(record[0])
      parsed_value = parsed_key.ParseValue(record[1])
      self.assertEqual(expected_key, parsed_key)
      self.assertEqual(parsed_value, expected_value)
    ...
    with self.subTest('KEY_PATH'):
      ...
      expected_value = chromium.IDBKeyPath(
    ...
    with self.subTest('MULTI_ENTRY_FLAG'):
      ...
      record = (b'\x00\x04\x00\x00d\x01\x1f\x03', b'\x00')
  ...
  def test_object_store_meta_data_key(self):
    ...
    with self.subTest('OBJECT_STORE_NAME'):
      expected_key = chromium.ObjectStoreMetaDataKey(
          offset=4,
          key_prefix=chromium.KeyPrefix(
              offset=0, database_id=4, object_store_id=0, index_id=0),
          object_store_id=1,
      ...
      parsed_key = chromium.ObjectStoreMetaDataKey.FromBytes(record[0])
      parsed_value = parsed_key.ParseValue(record[1])
      self.assertEqual(expected_key, parsed_key)
      self.assertEqual(parsed_value, expected_value)
    ...
    with self.subTest('HAS_KEY_PATH'):
      ...
      record = (b'\x00\x04\x00\x002\x01\x06', b'\x01')
    ...
    with self.subTest('KEY_GENERATOR_CURRENT_NUMBER'):
      ...
      record = (b'\x00\x04\x00\x002\x01\x07', b'\x01')
  ...
  def test_object_store_names_key(self):
    ...
    expected_key = chromium.ObjectStoreNamesKey(
        offset=4,
        key_prefix=chromium.KeyPrefix(
            offset=0, database_id=4, object_store_id=0, index_id=0),
    ...
    parsed_key = chromium.ObjectStoreNamesKey.FromBytes(record[0])
    parsed_value = parsed_key.ParseValue(record[1])
    self.assertEqual(expected_key, parsed_key)
    self.assertEqual(parsed_value, expected_value)
  ...
  def test_object_store_data_key(self):
    ...
    expected_key = chromium.ObjectStoreDataKey(
        offset=4,
        key_prefix=chromium.KeyPrefix(
    ...
    parsed_key = chromium.ObjectStoreDataKey.FromBytes(record[0])
    parsed_value = parsed_key.ParseValue(record[1])
    self.assertEqual(expected_key, parsed_key)
    self.assertEqual(parsed_value, expected_value)

    parsed_key = chromium.IndexedDbKey.FromBytes(record[0])
    self.assertEqual(parsed_key, expected_key)

  def test_exists_entry_key(self):
    """Tests the ExistsEntryKey."""
    expected_key = chromium.ExistsEntryKey(
        offset=4,
        key_prefix=chromium.KeyPrefix(
    ...
    record = (b'\x00\x01\x01\x02\x03\x00\x00\x00\x00\x00\x00\xf0?', b'\x02')

    parsed_key = chromium.ExistsEntryKey.FromBytes(record[0])
    parsed_value = parsed_key.ParseValue(record[1])
    self.assertEqual(expected_key, parsed_key)
    self.assertEqual(parsed_value, expected_value)
  ...
  def test_index_data_key(self):
    ...
    expected_key = chromium.IndexDataKey(
        offset=4,
        key_prefix=chromium.KeyPrefix(
        ...
        encoded_user_key=chromium.IDBKey(
            ...
            sequence_number=0,
            encoded_primary_key=chromium.IDBKey(
                offset=3720,
    ...
    parsed_key = chromium.IndexDataKey.FromBytes(record[0])
    parsed_value = parsed_key.ParseValue(record[1])
    self.assertEqual(expected_key, parsed_key)
    self.assertEqual(parsed_value, expected_value)
  ...
  def test_blob_entry_key(self):
    ...
    expected_key = chromium.BlobEntryKey(
        offset=4,
        key_prefix=chromium.KeyPrefix(
            offset=0, database_id=1, object_store_id=1, index_id=3),
        user_key=chromium.IDBKey(
            offset=4, type=definitions.IDBKeyType.NUMBER, value=3.0))
    expected_value = chromium.IndexedDBExternalObject(
        offset=0,
        entries=[
            chromium.ExternalObjectEntry(
                offset=0,
    ...
    parsed_key = chromium.BlobEntryKey.FromBytes(record[0])
    parsed_value = parsed_key.ParseValue(record[1])
    self.assertEqual(expected_key, parsed_key)
    self.assertEqual(parsed_value, expected_value)
