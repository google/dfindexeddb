...
class StreamDecoder:
  ...
  def DecodeSortableBinary(self) -> Tuple[int, bytes]:
    ...
    chunk_size = 8
    marker = chunk_size + 1
    empty_binary_sentinel = 0
    output = bytearray()

    offset, first = self.PeekBytes(1)
    if first[0] == empty_binary_sentinel:
      self.ReadBytes(1)
      return offset, b""

    while True:
      _, marker_or_sentinel = self.DecodeUint8()
      if marker_or_sentinel == marker:
        _, chunk = self.ReadBytes(chunk_size)
        output.extend(chunk)
        continue
      if 1 <= marker_or_sentinel <= chunk_size:
        payload_len = marker_or_sentinel
        padding_len = chunk_size - payload_len
        if padding_len > 0:
  ...
  def DecodeSortableString(self) -> Tuple[int, str]:
    ...
    while True:
      ...
      if first == 0:  # kSentinel
        break
      if (first & 0x80) == 0:
        output.append(chr((first & 0x7F) - 1))
      elif (first & 0xC0) == 0x80:  # kTwoByteEncodingIndicator
        _, second = self.DecodeUint8()
        output.append(chr(((first & 0x3F) << 8) | second))
      elif first == 0xFF:  # kThreeByteEncodingIndicator
